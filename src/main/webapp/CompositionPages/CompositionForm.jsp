...
<div class = "container" style="width: 600px; margin-top: 150px;">
    <form class="row g-3"  action="addcompositiontoalbum?id=${album.getId()}"  method="post">
        <div class="col-md-12" id = "errorPresented" style="margin-bottom: 9px; color:#f52342">
            <small class="text-muted"></small>
        </div>
        <c:if test ="${composition!=null}">
            <div class="col-md-12">
                <input type="text" style = "visibility: hidden" class="form-control" id="CompositionId" readonly name = "CompositionId" value = "<c:out value='${composition.getId()}'/>">
            </div>
        </c:if>
        <div class="col-md-12">
            <label for="AlbumName">Название альбома</label>
            <input type="text" class="form-control" id="AlbumName" name = "AlbumName" readonly value = "<c:out value='${name}'/>">
        </div>
        <div class="col-md-6">
            <label for="CompositionName">Название композиции</label>
            <input type="text" class="form-control" id="CompositionName" name = "CompositionName" placeholder="Название композиции" required = "required" <c:if test ="${composition!=null}"> value = "<c:out value='${composition.getName()}'/>"</c:if>>
            <c:choose>
                <c:when test="${composition!=null}">
                    <script>
                        var input = document.getElementById("CompositionName")
                        input.onblur = function()
                        {
                            lockSubmit(${composition.getId()})
                        };
                    </script>
                </c:when>
                <c:otherwise>
        ...
        <div class="col-md-6">
            <label for="CompositionName">Длительность композиции</label>
            <input type="number" min="1" max="3600000" class="form-control" id="CompositionDuration" name = "CompositionDuration" placeholder="Длительность композиции" required = "required" <c:if test ="${composition!=null}"> value = "<c:out value='${composition.getDuration()}'/>" </c:if>>
            <script>
                var input = document.getElementById("CompositionDuration")
                input.oninput = function()
                {
                    if (input.value > 3600000 || input.value < 1 || input.value == "." || input.value == "-" || input.value == "," || isNaN(input.value))
                        input.value=1;
                }
            </script>
        </div>

...
<div class = "container">
    <div class="row">
        <div class="col-sm-8"><a href="compositionalbumform?name=<c:out value="${album.getAlbumName()}"/>">Add composition to album</a></div>
    </div>
    <table class="table table-hover table-striped">
        ...
        <tbody>
        <c:forEach var="composition" items="${album.getCompostitionList()}">
            <tr>
                <td><c:out value="${album.getAlbumName()}"/></td>
                <td><c:out value="${composition.getName()}"/></td>
                <td><c:out value="${composition.getDuration()}"/></td>
                <td><a href="editcomposition?compositionname=<c:out value='${composition.getName()}'/>&name=<c:out value="${album.getAlbumName()}"/>">Edit</a></td>
                <td><a href="deletecomposition?compositionname=<c:out value='${composition.getName()}'/>&name=<c:out value="${album.getAlbumName()}"/>">Delete</a></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
    <div class="col-sm-8"><a href="albums">To all albums</a></div>
</div>
